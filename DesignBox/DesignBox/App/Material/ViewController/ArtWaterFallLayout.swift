...
@objc protocol ArtWaterFallLayoutDelegate {
    //waterFall的列数
    func columnOfWaterFall(_ collectionView: UICollectionView) -> Int
    //每个item的高度
    func waterFall(_ collectionView: UICollectionView, layout waterFallLayout: ArtWaterFallLayout, heightForItemAt indexPath: IndexPath) -> CGFloat
}


class ArtWaterFallLayout: UICollectionViewFlowLayout {
    //代理
    weak var delegate: ArtWaterFallLayoutDelegate?
    //行间距
